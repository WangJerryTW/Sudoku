<template>
  <div>
    ...
    <center>
      <table>
          <tr v-for="row in sudo.row">
              <td v-for="item in row" :class="[item.type,'qcolor']" v-if="item.ques" style="cursor:context-menu;">
                  {{item.grid_number}}<br>
              </td>
      ...
      <table>
        <tr>
          <td v-for="item in select_number" :class="[item.type,'qcolor']" @click="select(item)">{{ item.number }}</td>
        </tr>
      </table>
      <table>
        <tr>
